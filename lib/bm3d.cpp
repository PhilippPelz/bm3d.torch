 ...
 **/
 int run_bm3d(
     const float sigma
 ,   THFloatTensor *img_noisy1
 ,   THFloatTensor *img_basic1
 ,   THFloatTensor *img_denoised1
 ...
 ,   const unsigned color_space
    ...
    //  cout << "sigma " << sigma << " " << "width " << width << " " <<"height " << height << " " <<"chnls " << chnls << " " <<"useSD_h " << useSD_h << " " <<"useSD_w " << useSD_w << " " <<"tau_2D_hard " << tau_2D_hard << " " <<"tau_2D_wien " << tau_2D_wien << " "<<"color_space " << color_space << " " << endl;
     ...
     std::memcpy(img_noisy.data(),img_noisy1_d,img_noisy.size()*sizeof(float));
    ...
    //       return EXIT_FAILURE;
     ...
     int x =  run_bm3d1(sigma,img_noisy,img_basic,img_denoised,width,height,chnls,useSD_h,useSD_w,tau_2D_hard,tau_2D_wien,color_space);
    //  if (save_image("img_denoised2.png", img_denoised, width, height, chnls) != EXIT_SUCCESS)
    //         return EXIT_FAILURE;
            // if (save_image("img_basic2.png", img_basic, width, height, chnls) != EXIT_SUCCESS)
            //        return EXIT_FAILURE;
    //  cout << "Tensor size: img_basic1    " << THFloatTensor_nElement(img_basic1) << " " << "vector size: " << img_basic.size() << endl;
    //  cout << "Tensor size img_denoised1: " << THFloatTensor_nElement(img_denoised1) << " " << "vector size: " << img_denoised.size() << endl;
     memcpy(THFloatTensor_data(img_basic1),img_basic.data(),img_basic.size()*sizeof(float));
     memcpy(THFloatTensor_data(img_denoised1),img_denoised.data(),img_denoised.size()*sizeof(float));
...
,   const unsigned color_space
    ...
    if (nb_threads == 1)
    {
        ...
        // if (save_image("img_sym_basic0.png", img_sym_basic, w_b, h_b, chnls) != EXIT_SUCCESS)
        //        return EXIT_FAILURE;
    ...
    else
    {
        ...
        // if (save_image("img_sym_basic0.png", img_basic, width, height, chnls) != EXIT_SUCCESS)
        //        return EXIT_FAILURE;
    ...
    // cout<< "img_basic" <<endl;
        ...
        // if (save_image("img_denoised0.png", img_denoised, width, height, chnls) != EXIT_SUCCESS)
        //        return EXIT_FAILURE;
    }
    ...
    if (color_space_transform(img_denoised, color_space, width, height, chnls, false)
        != EXIT_SUCCESS) return EXIT_FAILURE;
        // if (save_image("img_denoised1.png", img_denoised, width, height, chnls) != EXIT_SUCCESS)
        //        return EXIT_FAILURE;
    ...
    if (color_space_transform(img_basic, color_space, width, height, chnls, false)
        != EXIT_SUCCESS) return EXIT_FAILURE;
        // if (save_image("img_basic1.png", img_basic, width, height, chnls) != EXIT_SUCCESS)
        //        return EXIT_FAILURE;
