...
bool ComparaisonFirst(pair<float, unsigned> pair1,
                      pair<float, unsigned> pair2) {
 ...
 **/
int run_bm3d(const float sigma, THFloatTensor *img_noisy1,
             THFloatTensor *img_basic1,
             THFloatTensor *img_denoised1, const unsigned width,
             const unsigned height, const unsigned chnls, const bool useSD_h,
             const bool useSD_w, const unsigned tau_2D_hard,
             const unsigned tau_2D_wien, const unsigned color_space) {
  cout << "sigma " << sigma << " " << "width " << width << " " <<"height " << height << " " <<"chnls " << chnls << " " <<"useSD_h " << useSD_h << " " <<"useSD_w " << useSD_w << " " <<"tau_2D_hard " << tau_2D_hard << " " <<"tau_2D_wien " << tau_2D_wien << " "<<"color_space " << color_space << " " << endl;

  float *img_noisy1_d = THFloatTensor_data(img_noisy1);
  std::vector<float> img_noisy(img_noisy1_d, img_noisy1_d + THFloatTensor_nElement(img_noisy1));
  std::memcpy(img_noisy.data(),img_noisy1_d,img_noisy.size());

    if (save_image("noisy.png", img_noisy, width, height, chnls) != EXIT_SUCCESS)
       return EXIT_FAILURE;
  ...
  if (color_space_transform(img_noisy, color_space, width, height, chnls,
                            true) != EXIT_SUCCESS)
    return EXIT_FAILURE;
    if (save_image("img_noisy_colorspace.png", img_noisy, width, height, chnls) != EXIT_SUCCESS)
       return EXIT_FAILURE;
...
// #endif
  ...
  if (nb_threads == 1) {
    ...
    if (save_image("img_sym_noisy.png", img_sym_noisy, w_b, h_b, chnls) != EXIT_SUCCESS)
       return EXIT_FAILURE;
    ...
    // cout << "done." << endl;
    if (save_image("img_sym_basic0.png", img_sym_basic, width, height, chnls) != EXIT_SUCCESS)
       return EXIT_FAILURE;
    ...
    if (save_image("img_sym_basic1.png", img_sym_basic, w_b, h_b, chnls) != EXIT_SUCCESS)
       return EXIT_FAILURE;
    //! Denoising, 2nd Step
    // cout << "step 2...";
    bm3d_2nd_step(sigma, img_sym_noisy, img_sym_basic, img_sym_denoised, w_b,
                  h_b, chnls, nWien, kWien, NWien, pWien, useSD_w, color_space,
                  tau_2D_wien, &plan_2d_for_1[0], &plan_2d_for_2[0],
                  &plan_2d_inv[0]);
    // cout << "done." << endl;
...
#pragma omp for schedule(dynamic) nowait
      for (unsigned n = 0; n < nb_threads; n++) {
        bm3d_1st_step(sigma, sub_noisy[n], sub_basic[n], w_table[n], h_table[n],
                      chnls, nHard, kHard, NHard, pHard, useSD_h, color_space,
                      tau_2D_hard, &plan_2d_for_1[n], &plan_2d_for_2[n],
                      &plan_2d_inv[n]);
      }
    }
    // cout << "done." << endl;
    ...
    //   cout<< "img_basic" <<endl;
    //   	for(int i =0;i<img_basic.size();i++)
    //   		cout << img_basic[i] << endl;
    //! Denoising: 2nd Step
    // cout << "step 2...";
...
#pragma omp for schedule(dynamic) nowait
    ...
    sub_divide(img_denoised, sub_denoised, w_table, h_table, width, height,
               chnls, 2 * nWien, false);
  }

  //! Inverse color space transform to RGB
  if (color_space_transform(img_denoised, color_space, width, height, chnls,
                            false) != EXIT_SUCCESS)
    return EXIT_FAILURE;
  if (color_space_transform(img_noisy, color_space, width, height, chnls,
                            false) != EXIT_SUCCESS)
    return EXIT_FAILURE;
  if (color_space_transform(img_basic, color_space, width, height, chnls,
                            false) != EXIT_SUCCESS)
    return EXIT_FAILURE;
    if (save_image("img_basic.png", img_basic, width, height, chnls) != EXIT_SUCCESS)
       return EXIT_FAILURE;
       if (save_image("img_denoised.png", img_denoised, width, height, chnls) != EXIT_SUCCESS)
          return EXIT_FAILURE;
    // cout<< "img_basic" <<img_basic.data()<<endl;
    // cout<< "img_denoised" <<img_denoised.data()<<endl;
    // for(int i =0;i<200;i++)
    //     cout << img_basic[i] << endl;
    // cout<< "img_denoised" <<endl;
    // for(int i =0;i<200;i++)
    //     cout << img_denoised[i] << endl;
  //! Free Memory
  ...
  fftwf_cleanup();
  std::memcpy(img_basic1_d,img_basic.data(),img_basic.size());
  std::memcpy(img_denoised1_d,img_denoised.data(),img_denoised.size());
 ...
 * @return none.
 **/
void bm3d_1st_step(const float sigma, vector<float> const &img_noisy,
                   vector<float> &img_basic, const unsigned width,
                   const unsigned height, const unsigned chnls,
                   const unsigned nHard, const unsigned kHard,
                   const unsigned NHard, const unsigned pHard, const bool useSD,
                   const unsigned color_space, const unsigned tau_2D,
                   fftwf_plan *plan_2d_for_1, fftwf_plan *plan_2d_for_2,
  ...
  const float tauMatch =
      ...
      (sigma_table[0] < 35.0f
           ? 2500
           : 5000); //! threshold used to determinate similarity between patches
  ...
  vector<float> hadamard_tmp(NHard);
    ...
    // std::cout << "bm3d_1st_step: "<< "img_noisy1 " <<  img_noisy.size() << "img_basic1 " <<  img_basic.size() << std::endl<< std::endl;
  ...
  precompute_BM(patch_table, img_noisy, width, height, kHard, NHard, nHard,
                pHard, tauMatch);

  //! table_2D[p * N + q + (i * width + j) * kHard_2 + c * (2 * nHard + 1) *
  //! width * kHard_2]
  ...
  for (unsigned ind_i = 0; ind_i < row_ind.size(); ind_i++) {
    ...
    if (tau_2D == DCT)
      ...
      bior_2d_process(table_2D, img_noisy, nHard, width, height, chnls, kHard,
                      i_r, pHard, row_ind[0], row_ind.back(), lpd, hpd);
        ...
        for (unsigned n = 0; n < nSx_r; n++) {
          ...
          for (unsigned k = 0; k < kHard_2; k++)
            group_3D[n + k * nSx_r + c * kHard_2 * nSx_r] =
                table_2D[k + ind * kHard_2 +
                         c * kHard_2 * (2 * nHard + 1) * width];
        }

      //! HT filtering of the 3D group
      vector<float> weight_table(chnls);
      ht_filtering_hadamard(group_3D, hadamard_tmp, nSx_r, kHard, chnls,
                            sigma_table, lambdaHard3D, weight_table, !useSD);
            ...
            for (unsigned q = 0; q < kHard; q++) {
              ...
              numerator[ind] += kaiser_window[p * kHard + q] *
                                ...
                                group_3D_table[p * kHard + q + n * kHard_2 +
                                               c * kHard_2 * nSx_r + dec];
              denominator[ind] +=
                  kaiser_window[p * kHard + q] * wx_r_table[c + ind_j * chnls];
            }
        }
      }
 ...
 * @return none.
 **/
void bm3d_2nd_step(const float sigma, vector<float> const &img_noisy,
                   vector<float> const &img_basic, vector<float> &img_denoised,
                   const unsigned width, const unsigned height,
                   const unsigned chnls, const unsigned nWien,
                   const unsigned kWien, const unsigned NWien,
                   const unsigned pWien, const bool useSD,
                   const unsigned color_space, const unsigned tau_2D,
                   fftwf_plan *plan_2d_for_1, fftwf_plan *plan_2d_for_2,
                   fftwf_plan *plan_2d_inv) {
  //! Estimatation of sigma on each channel
  ...
  const float tauMatch =
      (sigma_table[0] < 35.0f
           ? 400
           : 3500); //! threshold used to determinate similarity between patches
  ...
  vector<float> tmp(NWien);
    // std::cout << "bm3d_2nd_step: "<< "img_noisy1 " <<  img_noisy.size() << "img_basic1 " <<  img_basic.size()<< "img_denoised " <<  img_denoised.size() << std::endl<< std::endl;
  //! Check allocation memory
  if (img_denoised.size() != img_noisy.size()){
    img_denoised.resize(img_noisy.size());
  ...
  //! Precompute Bloc-Matching
  vector<vector<unsigned> > patch_table;
  precompute_BM(patch_table, img_basic, width, height, kWien, NWien, nWien,
                pWien, tauMatch);
  ...
  //! DCT_table_2D[p * N + q + (i * width + j) * kWien_2 + c * (2 * ns + 1) *
  //! width * kWien_2]
  ...
  for (unsigned ind_i = 0; ind_i < row_ind.size(); ind_i++) {
    ...
    if (tau_2D == DCT) {
      ...
      bior_2d_process(table_2D_img, img_noisy, nWien, width, height, chnls,
                      kWien, i_r, pWien, row_ind[0], row_ind.back(), lpd, hpd);
      bior_2d_process(table_2D_est, img_basic, nWien, width, height, chnls,
                      kWien, i_r, pWien, row_ind[0], row_ind.back(), lpd, hpd);
    }
        ...
        for (unsigned n = 0; n < nSx_r; n++) {
          ...
          for (unsigned k = 0; k < kWien_2; k++) {
            group_3D_est[n + k * nSx_r + c * kWien_2 * nSx_r] =
                table_2D_est[k + ind * kWien_2 +
                             c * kWien_2 * (2 * nWien + 1) * width];
            group_3D_img[n + k * nSx_r + c * kWien_2 * nSx_r] =
                table_2D_img[k + ind * kWien_2 +
                             c * kWien_2 * (2 * nWien + 1) * width];
          }
        }
            ...
            for (unsigned q = 0; q < kWien; q++) {
              ...
              numerator[ind] += kaiser_window[p * kWien + q] *
                                ...
                                group_3D_table[p * kWien + q + n * kWien_2 +
                                               c * kWien_2 * nSx_r + dec];
              denominator[ind] +=
                  kaiser_window[p * kWien + q] * wx_r_table[c + ind_j * chnls];
            }
        }
      }
 ...
 **/
void dct_2d_process(vector<float> &DCT_table_2D, vector<float> const &img,
                    fftwf_plan *plan_1, fftwf_plan *plan_2, const unsigned nHW,
                    const unsigned width, const unsigned height,
                    const unsigned chnls, const unsigned kHW,
                    const unsigned i_r, const unsigned step,
                    vector<float> const &coef_norm, const unsigned i_min,
  ...
  if (i_r == i_min || i_r == i_max) {
    ...
    //! Compute the new DCT
    float *vec =
        (float *)fftwf_malloc(chnls * kHW_2 * step * width * sizeof(float));
    float *dct =
        (float *)fftwf_malloc(chnls * kHW_2 * step * width * sizeof(float));
    ...
    for (unsigned c = 0; c < chnls; c++) {
      ...
      for (unsigned i = 0; i < step; i++)
        for (unsigned j = 0; j < width - kHW; j++)
          for (unsigned k = 0; k < kHW_2; k++)
            DCT_table_2D[dc + ((i + 2 * nHW + 1 - step) * width + j) * kHW_2 +
                         k] =
                dct[dc_p + (i * width + j) * kHW_2 + k] * coef_norm[k];
    }
    fftwf_free(dct);
 ...
 **/
void bior_2d_process(vector<float> &bior_table_2D, vector<float> const &img,
                     const unsigned nHW, const unsigned width,
                     const unsigned height, const unsigned chnls,
                     const unsigned kHW, const unsigned i_r,
                     const unsigned step, const unsigned i_min,
                     const unsigned i_max, vector<float> &lpd,
                     vector<float> &hpd) {
  //! Declarations
  const unsigned kHW_2 = kHW * kHW;
  ...
  if (i_r == i_min || i_r == i_max) {
    ...
    for (unsigned c = 0; c < chnls; c++) {
      ...
      for (unsigned i = 0; i < step; i++)
        for (unsigned j = 0; j < width - kHW; j++) {
          bior_2d_forward(
              img, bior_table_2D, kHW,
              dc + (i + 2 * nHW + 1 - step + i_r - nHW) * width + j, width,
              dc_p + ((i + 2 * nHW + 1 - step) * width + j) * kHW_2, lpd, hpd);
        }
    }
  }
}
 ...
 * @return none.
 **/
void ht_filtering_hadamard(vector<float> &group_3D, vector<float> &tmp,
                           const unsigned nSx_r, const unsigned kHard,
                           const unsigned chnls,
                           vector<float> const &sigma_table,
                           const float lambdaHard3D,
                           vector<float> &weight_table, const bool doWeight) {
  //! Declarations
  const unsigned kHard_2 = kHard * kHard;
  for (unsigned c = 0; c < chnls; c++)
    weight_table[c] = 0.0f;
  ...
  if (doWeight)
    for (unsigned c = 0; c < chnls; c++)
      weight_table[c] = (weight_table[c] > 0.0f
                             ? 1.0f / (float)(sigma_table[c] * sigma_table[c] *
                                              weight_table[c])
                             : 1.0f);
}
 ...
 * @return none.
 **/
void wiener_filtering_hadamard(vector<float> &group_3D_img,
                               vector<float> &group_3D_est, vector<float> &tmp,
                               const unsigned nSx_r, const unsigned kWien,
                               const unsigned chnls,
                               vector<float> const &sigma_table,
 ...
 * @return none.
 **/
void dct_2d_inverse(vector<float> &group_3D_table, const unsigned kHW,
                    const unsigned N, vector<float> const &coef_norm_inv,
                    fftwf_plan *plan) {
  //! Declarations
  const unsigned kHW_2 = kHW * kHW;
...
void bior_2d_inverse(vector<float> &group_3D_table, const unsigned kHW,
                     vector<float> const &lpr, vector<float> const &hpr) {
  //! Declarations
  const unsigned kHW_2 = kHW * kHW;
 ...
 * @return none.
 **/
void preProcess(vector<float> &kaiserWindow, vector<float> &coef_norm,
                vector<float> &coef_norm_inv, const unsigned kHW) {
 ...
 * @return none.
 **/
void precompute_BM(vector<vector<unsigned> > &patch_table,
                   const vector<float> &img, const unsigned width,
                   const unsigned height, const unsigned kHW,
                   const unsigned NHW, const unsigned nHW, const unsigned pHW,
                   const float tauMatch) {
  //! Declarations
  const unsigned Ns = 2 * nHW + 1;
  const float threshold = tauMatch * kHW * kHW;
  vector<float> diff_table(width * height);
  vector<vector<float> > sum_table((nHW + 1) * Ns,
                                  vector<float>(width * height, 2 * threshold));
  ...
  for (unsigned di = 0; di <= nHW; di++)
    for (unsigned dj = 0; dj < Ns; dj++) {
      ...
      for (unsigned j = nHW + 1; j < width - nHW; j++) {
        ...
        for (unsigned p = 0; p < kHW; p++)
          sum +=
              diff_table[ind + p * width + kHW] - diff_table[ind + p * width];
      ...
      for (unsigned i = nHW + 1; i < height - nHW; i++) {
        ...
        for (unsigned j = nHW + 1; j < width - nHW; j++, k++, pq++) {
          sum_table[ddk][k] =
              sum_table[ddk][k - 1] + sum_table[ddk][k - width] -
              sum_table[ddk][k - 1 - width] + diff_table[pq] -
              diff_table[pq - kHW] - diff_table[pq - kHW * width] +
    ...
    for (unsigned ind_j = 0; ind_j < column_ind.size(); ind_j++) {
      ...
      for (int dj = -(int)nHW; dj <= (int)nHW; dj++) {
        ...
        for (int di = -(int)nHW; di < 0; di++)
          if (sum_table[-dj + nHW + (-di) * Ns][k_r] < threshold)
            table_distance.push_back(make_pair(sum_table[-dj + nHW + (-di) * Ns]
                                                        [k_r + di * width + dj],
                                               k_r + di * width + dj));
      }
      ...
      const unsigned nSx_r = (NHW > table_distance.size()
                                  ? closest_power_of_2(table_distance.size())
                                  : NHW);
 ...
 * @return none.
 **/
void sd_weighting(std::vector<float> const &group_3D, const unsigned nSx_r,
                  const unsigned kHW, const unsigned chnls,
